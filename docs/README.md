...
| [39-resultado-gate-3-identidade-assinatura-warp.md](39-resultado-gate-3-identidade-assinatura-warp.md) | Resultado (2P-E-C3) da execução do **GATE 3** (identidade e assinatura estática **offline**): `GATE 3 CONCLUÍDO — IDENTIDADE CONFIRMADA E ASSINATURA DETERMINADA (AUSENTE)` (`COMPLETED_PASS`); rematerialização temporária do blob fixado, identidade **igual** ao GATE 2 (tamanho/Git OID/SHA-256), PE32 x86 válido, `OriginalFilename` `WARP.exe`, **Certificate Table ausente** (sem assinatura Authenticode; achado material, **não** malware); separação presença × validade × confiança × segurança; arquivo removido; **GATE 4 não autorizado** | Cliente/build/segurança | validado | concluído (identidade/assinatura) | 16, 33, 35, 37 | 2026-08-03 |
